...
const EditInvoice = () => {
  ...
  const handleUpdateInvoice = async () => {
    try {
      ...
      // update inventory
      // 1. Get quantity which was reduced while creating invoice and add it back to inventory
      await updateInventory("add");

      // 2. Update inventory by reducing quantity
      await updateInventory("reduce");

      const codeDoc = doc(db, USERS, uid, INVOICE_INFO, id);
    ...
    if (action === "add") {
      ...
      updatedItems = existingItems.map((item) => {
        ...
        if (row) {
          return {
            ...item,
            itemQty: parseInt(item.itemQty) + parseInt(row.qty),
          };
        }
        return item;
      });
  ...
  useEffect(() => {
    clearLocalStorage();
    // get invoice info for respective id

    const dashboardInfo = localStorage.getItem("dashboardInfo");
    if (!dashboardInfo) return;

    const result = JSON.parse(dashboardInfo);
    const invoiceData = result.filter((x) => x.id === id)[0];

    // Customer Info
    setCustomerName(invoiceData?.customerInfo?.customerName);
    setCustomerPhone(invoiceData?.customerInfo?.customerPhone);

    // Invoice Info
    setInvoiceNumber(invoiceData?.invoiceInfo?.invoiceNumber);
    setExpectedDate(invoiceData?.invoiceInfo?.expectedDate);

    // Vehicle Info
    setVehicleNumber(invoiceData?.vehicleInfo?.vehicleNumber);
    setVehicleKM(invoiceData?.vehicleInfo?.vehicleKM);
    setVehicleType(invoiceData?.vehicleInfo?.vehicleType);

    // Rows Info
    setRows(invoiceData?.rows);
    const newArray = structuredClone(invoiceData?.rows);
    setExistingRows(newArray);

    // Payment Type
    setPaymentType(invoiceData?.amountInfo?.paymentType);
    setAdvanceAmount(invoiceData?.amountInfo?.advance);
    setAmount(invoiceData?.amountInfo?.amount);
    // Discount
    setDiscountAmount(invoiceData?.amountInfo?.discount);
